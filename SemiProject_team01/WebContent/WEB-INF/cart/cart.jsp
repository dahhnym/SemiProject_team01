    ...
    $("button.option_change").bind("click", function(){
    	...
    	$.ajax({
			...
			success:function(json){
				    ...
				    var html = "<option value='0'>- [필수] 옵션을 선택해 주세요 -</option>";
				 	...
				 	// 레이어 팝업에서 '변경' 버튼 클릭시
				  	$("button.optionChange").bind("click",function(){
				  		...
				  		$.ajax({
							...
							type: "post",
							data: {"userid":"${sessionScope.loginuser.userid}"
								  , "newpdetailnum":newpdetailnum
								  , "fk_pnum":fk_pnum
								  , "oldpdetailnum":oldpdetailnum},
							dataType: "json",
							success:function(json){
								if(json.n==1){
									location.href="javascript:history.go(0);";
			   ...
			   var html = "";
				if(json.length > 0){	
			    	$.each(json, function(index, item){
						...
						html+="<input type='hidden' name='pname' value='"+item.pname+"'/>";
					...
					<c:if test="${not empty requestScope.cartList}">
						...
						<c:forEach items="${requestScope.cartList}" var="cart" varStatus="status">
									...
									<tr class="xans-record-">
										...
										<a href="<%=ctxPath%>/product/Info.to?pnum=${cart.fk_pnum}">
		...
		<table class="wishlist_table">
				...
				<thead>
					...
					<c:if test="${not empty requestScope.wishPageList}">
						...
						<tr>
						<td scope="col" class="thumb">
									<a href="<%=ctxPath%>/product/Info.to?pnum=${wish.fk_pnum}">
					...
					<!-- 옵션변경 레이어 -->
						...
						<!-- 옵션변경 레이어 -->
									...
									<td scope="col" class="total">
										...
										<c:if test="${wish.pvo.saleprice < 50000}"> 
												<strong><fmt:formatNumber value="${wish.pvo.saleprice+2500}" type="number" /> 원</strong>
										</c:if>
									</td>
							    	<td scope="col">
