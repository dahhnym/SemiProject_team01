			...
			if(!"POST".equalsIgnoreCase(method)) {
				...
				System.out.println(session);
				  ...
				  try {
		                ...
		                request.setAttribute("loc", request.getContextPath()+"/admin/productRegister.to"); 
	           ...
	           if(n*k*m==1) { // n은 제품등록(제품테이블에 insert완료)된 값 * m은 추가이미지(추가이미지 테이블에 insert 완료)등록 된 값
		              message = "제품등록 성공!!";
		              loc = request.getContextPath()+"/admin/productRegister.to";
		       }
	           else {
	              message = "제품등록 실패!!";
	              loc = request.getContextPath()+"/admin/productRegister.to";
...
=======
	      ...
	      //   super.setRedirect(false);
	         super.setViewPage("/WEB-INF/msg.jsp");
>>>>>>> branch 'main' of https://github.com/valentinakim/SemiProject_team01.git
		}
