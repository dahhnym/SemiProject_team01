...
public class CartDAO implements InterCartDAO {
	...
	// 제품 정보 알아오기
	@Override
	public List<CartVO> selectProdInfo(String pdetailnum) throws SQLException {
		
		List<CartVO> prodList = new ArrayList<>();
		
		try {
			
			conn = ds.getConnection();
			
			String sql = "select C.fk_pnum, P.pname, C.oqty, D.optionname, P.saleprice, C.fk_pdetailnum, C.cartnum, P.pimage1 "+
						"from tbl_cart C join tbl_product P  "+
						"on C.fk_pnum = P.pnum  "+
						"join "+
						"tbl_proddetail D "+
						"on C.fk_pdetailnum = D.pdetailnum "+
						"where C.fk_pdetailnum = ?";
			...
			if(rs.next()) {
				...
				int fk_pdetailnum = rs.getInt(6);
				int cartnum=rs.getInt(7);
				String pimage1=rs.getNString(8);
				
				CartVO cartvo = new CartVO();  
				cartvo.setFk_pnum(pnum);
				cartvo.setOqty(oqty);
				cartvo.setFk_pdetailnum(fk_pdetailnum);
				cartvo.setCartnum(cartnum);
	...
	// 위시리스트 제품 정보 알아오기
	@Override
	public List<WishListVO> selectWishProdInfo(String pdetailnum) throws SQLException {


		List<WishListVO> wprodList = new ArrayList<>();
		
		try {
			
			conn = ds.getConnection();
			
			String sql = "select W.fk_pnum, P.pname, W.oqty, D.optionname, P.saleprice, W.fk_pdetailnum, W.wnum, P.pimage1 "+
						"from tbl_wishlist W join tbl_product P  "+
						"on W.fk_pnum = P.pnum  "+
						"join "+
						"tbl_proddetail D "+
						"on W.fk_pdetailnum = D.pdetailnum "+
						"where W.fk_pdetailnum = ?";
