...
@WebServlet(
		description = "사용자가 웹에서 *.up을 했을 경우 응답해주는 서블릿", 
		urlPatterns = { "*.up" }, 
		initParams = { 
				@WebInitParam(name = "propertyConfig", value = "C:/SemiProject_team01/SemiProject_team01/WebContent/WEB-INF/Command.properties", description = "*.up에 대한 클래스의 매핑파일")
		})
public class FrontControllerServlet extends HttpServlet {
	...
	/*웹브라우저 주소창에서  *.up 을 하면 FrontControllerServelet 서블릿이 응대를 해오는데 맨 처음에 자동적으로 실행되어지는 메소드가 init(ServletConfig config) 이다.
	...
	//	System.out.println("init 메소드 실행됨");
		...
		//특정 파일에 있는 내용을 읽어오기 위한 용도로 쓰이는 객체. 여기서는 Command.properties 파일을 읽어오기 위해서 만들었다
		
		String props = config.getInitParameter("propertyConfig");
		//System.out.println("확인용 props "+props);
		// 확인용 props C:/NCS/workspace(jsp)/MyMVC/WebContent/WEB-INF/Command.properties 파일의 내용을 읽어오기 위한 용도로 쓰이는 객체
		
		try {
			fis = new FileInputStream(props);
			// fis는 C:/NCS/workspace(jsp)/MyMVC/WebContent/WEB-INF/Command.properties 파일의 내용을 읽어오기 위한 용도로 쓰이는 객체
		
			Properties pr = new Properties();
			// Properties 는 Collection 중 HashMap 계열중의  하나로써 "key","value"으로 이루어져 있는것이다.
			// 중요한 것은 Properties 는 key도 String 타입이고, value도 String 타입만 가능하다는 것이다.
			...
			/* pr.load(fis); 은  fis 객체를 사용하여 C:/NCS/workspace(jsp)/MyMVC/WebContent/WEB-INF/Command.properties 파일의 내용을 읽어다가 
	         Properties 클래스의 객체인 pr 에 로드시킨다. 그러면 pr 은 읽어온 파일(Command.properties)의 내용에서 = 을 기준으로 왼쪽은 key로 보고, 오른쪽은 value 로 인식한다.*/
			
			Enumeration<Object> en = pr.keys();
			//pr.keys();은 C:/NCS/workspace(jsp)/MyMVC/WebContent/WEB-INF/Command.properties 파일의 내용물에서 =을 기준으로 왼쪽에 있는 모든 key 들만 가져오는 것이다.
			...
			while(en.hasMoreElements()) {
				...
				// cmdMap 에서 키값으로 Command.properties 파일에 저장되어진 url 을 주면 cmdMap 에서 해당 클래스에 대한 객체(인스턴스)를 얻어오도록 만든 것이다.
	...
	private void requestProcess(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// 웹브라우저의 주소 입력창에서 http://localhost:9090/MyMVC/member/idDuplicateCheck.up?userid=leess 와 같이 입력한 경우
		//String url = request.getRequestURL().toString();
		//System.out.println("확인용 : "+url);
		//확인용 : http://localhost:9090/MyMVC/member/idDuplicateCheck.up
		...
		*/
		...
		if(action == null) {
			...
			try {
		        ...
		        // 또한 forward 방식은 forward 되어지는 페이지로 데이터를 전달하는 것이다.
				...
				// viewPage 에 명기된 주소로 sendRedirect(웹브라우저의 URL주소 변경됨)를 하겠다는 말이다. 즉, 단순히 페이지이동을 하겠다는 말이다. 
		        // (!중요) sendRedirect 방식은 sendRedirect 되어지는 페이지로 데이터를 전달할 수가 없다. 	
